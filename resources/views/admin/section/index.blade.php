...
@section('content')
    <div class="container">
    <h3 class="mt-3 fw-bold">Разделы сайта</h3>
        <p class="text-right"><a class="btn btn-dark" href="{{route('section.create')}}">Добавить страницу</a></p>
...
@foreach($pages AS $page)
    <tr>
        <td>{{$page->id}}</td>
        <td><div class="iconname"><a class="folderpage fas fa-folder" href="{{route('page.index',['section'=>$page->id])}}"><small>{{$page->pages_count}}</small></a> <div><div class="font-bold">{{$page->title}}</div> <a href="/{{$page->url}}" class="small text-muted url">/{{$page->url}}</a></div> </div></td>
        ...
        <td><a class="btn btn-dark" href="{{route('section.edit',[$page->id])}}"><i class="fas fa-pencil-alt"></i></a></td>
        <td><form method="post" action="{{route('section.destroy',[$page->id])}}" onsubmit="return delItem(this)" data-sistem="{{$page->is_sistem}}"> @csrf @method('delete')<button type="submit" class="btn btn-danger fas fa-trash-alt"></button></form></td>
    </tr>
@endforeach
        </tbody>
    </table>
