...
@section('content')
    ...
    <h3 class="mt-3 fw-bold">Страницы сайта</h3>
        <p class="text-right"><a class="btn btn-dark" href="{{route('infopage.create')}}">Добавить страницу</a></p>
...
@foreach($pages AS $page)
    <tr>
        <td>{{$page->id}}</td>
        <td><div class="fw-bold">{{$page->title}}</div><a href="/{{$page->url}}" class="small text-muted">{{$page->url}}</a></td>
        ...
        <td><a class="btn btn-dark" href="{{route('infopage.edit',['id'=>$page->id])}}"><i class="fas fa-pencil-alt"></i></a></td>
        <td><form method="post" action="{{route('infopage.destroy',['id'=>$page->id])}}" onsubmit="return delItem(this)" data-sistem="{{$page->is_sistem}}"> @csrf @method('delete')<button type="submit" class="btn btn-danger fas fa-trash-alt"></button></form></td>
    </tr>
@endforeach
        </tbody>
    </table>
