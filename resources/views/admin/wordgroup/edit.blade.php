...
@section('content')
    ...
    <form method="POST" action="{{route('wordgroup.update',$item->id)}}">
        @csrf
        {{ method_field('PUT') }}
        <div class="container">
            ...
            <div class="card card-body mt-3 mb-4">
                ...
                <div class="row">
                    <div class="col-lg-4 text-center">
                        @if($item->image)
                            <div class="imgblock mt-4"><img class="img-fluid" src="{{asset($item->image)}}?{{time()}}" id="imagefile" alt=""> </div>
                        ...
                        <div id="configImg" data-id="{{$item->id}}" data-width="375" data-height="375" data-patch="wordgroup"></div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="title" class="form-control @error('name') is-invalid @enderror" value="{{old('title',$item->name)}}" required>
                    @error('title') <div class="text-danger">{{ $message }}</div> @enderror
                </div>
                <div class="mb-3">
                    <label for="title" class="form-label">Url</label>
                    <div class="input-group">
                        <input type="text" name="url" id="url" data-sistem="0" class="form-control @error('url') is-invalid @enderror" value="{{old('url',$item->url)}}" disabled="disabled" required>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="content">{{old('description',$item->description)}}</textarea>
            ...
            <div class="card card-body mt-3">
                <div class="mb-3">
                    ...
                    <div class="mt-2"><textarea id="editor" class="form-control" name="content" rows="15">{{$item->content}}</textarea></div> <div id="code"></div>
                </div>
    ...
    <script src="/assets/js/ckeditor/AjexFileManager/ajex.js" type="text/javascript"></script>
    <script src="assets/js/ajaxupload.js"></script>
    <script type="text/javascript">
        var ckeditor1 = CKEDITOR.replace('editor'); AjexFileManager.init({returnTo: 'ckeditor',editor: ckeditor1});
    </script>
@endsection
