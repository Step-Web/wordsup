    ...
    <div class="slider__wrapper phrases" id="slider">
       @php $wk=0; @endphp
        @foreach ($phrases AS $val)
            @php $val['phrase'] = rtrim($val['phrase'],'.');
                $variants =  explode(' ',$val['phrase']);
                @endphp
        <div class="slider__item" id="{{$val['id']}}" data-num="{{$wk}}" data-type="collatewords" data-correct="{{$val['phrase']}}" data-audio="@if($val['audio'] > 0){{($val['tID'] > 0)?$val['tID']:'s'.$val['tID']}}@endif">
            <span class="stat l{{$val['progress']??0}}" data-start="{{$val['progress']??0}}"></span>
            <div class="question">{{$val['translate']}}</div>
            <div class="falseVariant fs-3 mt-4 mb-3"><small class="text-muted fs-6">Составьте предложение из этих слов</small></div>
            <div class="answer trueVariant invisible text-success fs-5 mt-2">{{rtrim($val['phrase'])}}</div>
            <div class="variants">
                @foreach($variants AS $v)
                    <div class="variant" onclick="wordVariant(this)">{{$v}}</div>
                @endforeach
            </div>
        </div>
            @php $wk++; @endphp
