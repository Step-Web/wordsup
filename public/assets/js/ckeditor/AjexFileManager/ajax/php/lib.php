...
function getSize($dirFile, $mode = 'file')
{
	...
	if (empty($stats['size'])) {
		...
		$stats['_size'] .= ' B';
	} elseif ($stats['size'] < 5242880) {
		$stats['_size'] = round($stats['size'] / 1024) . ' KB';
	} else {
		$stats['_size'] = round($stats['size'] / 5242880, 2) . ' MB';
	}
...
function cropImg($dir_upload,$filename,$iWidth,$iHeight) {

        $iJpgQuality = 100;
		
$imgname= substr($dir_upload.$filename, 0, strrpos($dir_upload.$filename, '.'));
			      // new unique filename
                 $sTempFileName = $dir_upload.$filename;

                    // move uploaded file into cache folder
                    //move_uploaded_file($dir_upload.$_POST['image'], $sTempFileName);
